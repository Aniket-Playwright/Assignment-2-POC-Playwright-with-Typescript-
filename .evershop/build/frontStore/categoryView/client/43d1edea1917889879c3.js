...
`));
//# sourceMappingURL=SearchBox.js.map
;// ./node_modules/@evershop/evershop/dist/components/frontStore/catalog/CategoryContext.js

const CategoryContext = (0,react.createContext)(undefined);
const CategoryProvider = ({ children, category }) => {
    return (react.createElement(CategoryContext.Provider, { value: category }, children));
};
const useCategory = () => {
    const context = (0,react.useContext)(CategoryContext);
    if (context === undefined) {
...
;// ./node_modules/@evershop/evershop/dist/components/frontStore/catalog/CategoryInfo.js
...
function CategoryInfo() {
    ...
    return (react.createElement(react.Fragment, null,
        react.createElement(common_Area, { id: "beforeCategoryInfo", noOuter: true }),
        react.createElement("div", { className: "mb-2 md:mb-5 category__general" },
            image && (react.createElement(Image, { className: "category__image mb-5", src: image.url, alt: image.alt || name, width: 1800, height: 1029, priority: true })),
            react.createElement("div", { className: "category__info prose prose-base page-width" },
        ...
        react.createElement(common_Area, { id: "afterCategoryInfo", noOuter: true })));
}
...
;// ./node_modules/@evershop/evershop/dist/components/frontStore/catalog/CategoryProducts.js
...
function CategoryProducts() {
    ...
    return (react.createElement(react.Fragment, null,
        react.createElement(common_Area, { id: "categoryProductsBefore", className: "category__products__before" }),
        react.createElement("div", null,
            react.createElement(ProductList, { products: products.items, layout: "grid", gridColumns: 3, showAddToCart: true }),
            react.createElement("span", { className: "product-count italic block mt-5" }, _('${count} products', { count: products.total.toString() }))),
        react.createElement(common_Area, { id: "categoryProductsAfter", className: "category__products__after" })));
}
//# sourceMappingURL=CategoryProducts.js.map
;// ./node_modules/@evershop/evershop/dist/components/frontStore/catalog/ProductFilter.js


// Create a context for filter dispatch
const ProductFilterDispatch = react.createContext(null);
// Hook to use the filter context
const useProductFilter = () => {
    const context = (0,react.useContext)(ProductFilterDispatch);
    if (!context) {
        throw new Error('useProductFilter must be used within a ProductFilterProvider');
    }
    return context;
};
const ProductFilter = ({ currentFilters, availableAttributes = [], priceRange, categories = [], setting, onFilterUpdate, children }) => {
    ...
    const addFilter = (0,react.useCallback)((key, operation, value) => {
        let newFilters;
    ...
    const removeFilter = (0,react.useCallback)((key) => {
        const newFilters = currentFilters.filter((f) => f.key !== key);
        updateFilter(newFilters);
    }, [currentFilters, updateFilter]);
    const removeFilterValue = (0,react.useCallback)((key, value) => {
        ...
        if (filter) {
            ...
            if (newValues.length === 0) {
                ...
                const newFilters = currentFilters.map((f) => f.key === key && f.operation === 'in'
                    ? { key, operation: 'eq', value: newValues[0] }
                    : f);
                updateFilter(newFilters);
            }
            else {
    ...
    const toggleFilter = (0,react.useCallback)((key, operation, value) => {
    ...
    const clearAllFilters = (0,react.useCallback)(() => {
        setIsLoading(true);
        const url = new URL(window.location.href, window.location.origin);
    ...
    const hasFilter = (0,react.useCallback)((key) => {
        return currentFilters.some((f) => f.key === key);
    }, [currentFilters]);
    const getFilterValue = (0,react.useCallback)((key) => {
        var _a;
        return (_a = currentFilters.find((f) => f.key === key)) === null || _a === void 0 ? void 0 : _a.value;
    }, [currentFilters]);
    const isOptionSelected = (0,react.useCallback)((attributeCode, optionId) => {
        const filter = currentFilters.find((f) => f.key === attributeCode);
        return filter
            ? filter.value.split(',').includes(optionId.toString())
            : false;
    }, [currentFilters]);
    const isCategorySelected = (0,react.useCallback)((categoryId) => {
        const filter = currentFilters.find((f) => f.key === 'cat');
        return filter ? filter.value.split(',').includes(categoryId) : false;
    }, [currentFilters]);
    const getSelectedCount = (0,react.useCallback)((attributeCode) => {
        const filter = currentFilters.find((f) => f.key === attributeCode);
        return filter ? filter.value.split(',').length : 0;
    }, [currentFilters]);
    const getCategorySelectedCount = (0,react.useCallback)(() => {
        const filter = currentFilters.find((f) => f.key === 'cat');
        return filter ? filter.value.split(',').length : 0;
    }, [currentFilters]);
    const activeFilterCount = currentFilters.filter((f) => !['page', 'limit', 'ob', 'od'].includes(f.key)).length;
    const renderProps = {
        currentFilters,
        availableAttributes,
        priceRange,
...
;// ./node_modules/@evershop/evershop/dist/components/frontStore/catalog/DefaultAttributeFilterRender.js



const DefaultAttributeFilterRender = ({ availableAttributes, currentFilters }) => {
    const { updateFilter } = useProductFilter();
    const [searchTerms, setSearchTerms] = (0,react.useState)({});
    const [collapsedAttributes, setCollapsedAttributes] = (0,react.useState)({});
    ...
    return (react.createElement(react.Fragment, null, availableAttributes.map((attribute) => {
        ...
        return (react.createElement("div", { key: attribute.attributeCode, className: "attribute__filter__section border-b border-gray-200 pb-2 mb-2" },
            react.createElement("div", { className: "filter__header flex items-center justify-between mb-3" },
                react.createElement("button", { onClick: () => toggleCollapse(attribute.attributeCode), className: "flex items-center space-x-2 text-left flex-1 hover:text-blue-600 transition-colors" },
                    react.createElement("span", { className: "font-medium" }, attribute.attributeName),
                    selectedCount > 0 && (react.createElement("span", { className: "bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded-full" }, selectedCount)),
                    react.createElement("svg", { className: `w-4 h-4 transition-transform ${isCollapsed ? 'rotate-180' : ''}`, fill: "none", stroke: "currentColor", viewBox: "0 0 24 24" },
                        react.createElement("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M19 9l-7 7-7-7" }))),
                selectedCount > 0 && (react.createElement("button", { onClick: () => clearAttributeFilter(attribute.attributeCode), className: "text-gray-400 hover:text-red-500 text-sm transition-colors", title: "Clear all" }, "\u2715"))),
            !isCollapsed && (react.createElement("div", { className: "filter__content" },
                attribute.options.length > 5 && (react.createElement("div", { className: "mb-3" },
                    react.createElement("input", { type: "text", placeholder: `Search ${attribute.attributeName.toLowerCase()}...`, value: searchTerms[attribute.attributeCode] || '', onChange: (e) => setSearchTerms((prev) => ({
                            ...prev,
                            [attribute.attributeCode]: e.target.value
                        })), className: "w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" }))),
                react.createElement("div", { className: "attribute__options space-y-2 max-h-48 overflow-y-auto" }, filteredOptions.length > 0 ? (filteredOptions.map((option) => {
                    const isSelected = isOptionSelected(attribute.attributeCode, option.optionId.toString());
                    return (react.createElement("label", { key: option.optionId, className: `flex items-center space-x-3 cursor-pointer p-2 rounded hover:bg-gray-50 transition-colors ${isSelected
                            ? 'bg-blue-50 border border-blue-200'
                            : ''}` },
                        react.createElement("input", { type: "checkbox", checked: isSelected, onChange: (e) => handleAttributeChange(attribute.attributeCode, option.optionId.toString(), e.target.checked), className: "form-checkbox h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded" }),
                        react.createElement("span", { className: `text-sm ${isSelected
                                ? 'font-medium text-blue-900'
                                : 'text-gray-700'}` }, option.optionText)));
                })) : (react.createElement("div", { className: "text-gray-500 text-sm text-center py-4" }, _('No options found for "${code}"', {
                    code: searchTerms[attribute.attributeCode]
                })))),
                !searchTerms[attribute.attributeCode] &&
                    attribute.options.length > 10 && (react.createElement("button", { className: "text-blue-600 text-sm mt-2 hover:underline" }, _('Show all ${count} options', {
                    count: attribute.options.length.toString()
                })))))));
    })));
};
//# sourceMappingURL=DefaultAttributeFilterRender.js.map
;// ./node_modules/@evershop/evershop/dist/components/frontStore/catalog/DefaultCategoryFilterRender.js



const DefaultCategoryFilterRender = ({ categories, currentFilters }) => {
    const { updateFilter } = useProductFilter();
    ...
    return (react.createElement("div", { className: "category__filter__section border-b border-gray-200 pb-2 mb-2" },
        react.createElement("div", { className: "filter__header flex items-center justify-between mb-3" },
            react.createElement("button", { onClick: () => setIsCollapsed(!isCollapsed), className: "flex items-center space-x-2 text-left flex-1 hover:text-blue-600 transition-colors" },
                react.createElement("span", { className: "font-medium" }, "Categories"),
                selectedCount > 0 && (react.createElement("span", { className: "bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded-full" }, selectedCount)),
                react.createElement("svg", { className: `w-4 h-4 transition-transform ${isCollapsed ? 'rotate-180' : ''}`, fill: "none", stroke: "currentColor", viewBox: "0 0 24 24" },
                    react.createElement("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M19 9l-7 7-7-7" }))),
            selectedCount > 0 && (react.createElement("button", { onClick: clearCategoryFilter, className: "text-gray-400 hover:text-red-500 text-sm transition-colors", title: "Clear categories" }, "\u2715"))),
        !isCollapsed && (react.createElement("div", { className: "filter__content" },
            categories.length > 5 && (react.createElement("div", { className: "mb-3" },
                react.createElement("input", { type: "text", placeholder: "Search categories...", value: searchTerm, onChange: (e) => setSearchTerm(e.target.value), className: "w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" }))),
            react.createElement("div", { className: "category__options space-y-2 max-h-48 overflow-y-auto" }, filteredCategories.length > 0 ? (filteredCategories.map((category) => {
                const isSelected = isCategorySelected(category.categoryId.toString());
                return (react.createElement("label", { key: category.categoryId, className: `flex items-center space-x-3 cursor-pointer p-2 rounded hover:bg-gray-50 transition-colors ${isSelected ? 'bg-blue-50 border border-blue-200' : ''}` },
                    react.createElement("input", { type: "checkbox", checked: isSelected, onChange: (e) => handleCategoryChange(category.categoryId.toString(), e.target.checked), className: "form-checkbox h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded" }),
                    react.createElement("span", { className: `text-sm ${isSelected
                            ? 'font-medium text-blue-900'
                            : 'text-gray-700'}` }, category.name)));
            })) : (react.createElement("div", { className: "text-gray-500 text-sm text-center py-4" }, _('No categories found for "${term}"', { term: searchTerm }))))))));
};
//# sourceMappingURL=DefaultCategoryFilterRender.js.map
// EXTERNAL MODULE: ./node_modules/react-range-slider-input/dist/index.js
var dist = __webpack_require__(22136);
;// ./node_modules/@evershop/evershop/dist/components/frontStore/catalog/DefaultFilterWrapperRender.js

const DefaultFilterWrapperRender = ({ title, children }) => (react.createElement("div", { className: "filter__section" },
    react.createElement("div", { className: "filter__title font-medium mb-3" }, title),
    react.createElement("div", { className: "filter__content" }, children)));
//# sourceMappingURL=DefaultFilterWrapperRender.js.map
;// ./node_modules/@evershop/evershop/dist/components/frontStore/catalog/DefaultPriceFilterRender.js
...
const DefaultPriceFilterRender = ({ priceRange, currentFilters, setting }) => {
    ...
    return (react.createElement(DefaultFilterWrapperRender, { title: _('Price') },
        react.createElement("div", { className: "price__filter border-b border-gray-200 pb-2 mb-2" },
            react.createElement("div", { className: "price__slider mb-4" },
                react.createElement(dist/* default */.A, { min: priceRange.min, max: priceRange.max, value: [localMin, localMax], onInput: handleRangeChange })),
            react.createElement("div", { className: "flex justify-between text-small text-gray-500 mt-2" },
                react.createElement("span", null, priceRange.minText),
                react.createElement("span", null, priceRange.maxText))),
        react.createElement("style", null, `.range-slider .range-slider__thumb { width: 1rem; height: 1rem} .range-slider {height:6px}`)));
};
//# sourceMappingURL=DefaultPriceFilterRender.js.map
;// ./node_modules/@evershop/evershop/dist/components/frontStore/catalog/DefaultProductFilterSummary.js


const formatPrice = (oldFormatted, price) => {
    const match = oldFormatted.match(/^[^\d.,]+/);
    const currencySymbol = match ? match[0] : '';
    return currencySymbol + price;
};
const getFilterSummary = (availableAttributes, currentFilters, priceRange, categories) => {
    const summaries = [];
    // Price filters
    ...
    if (minPrice || maxPrice) {
        ...
        const max = (maxPrice === null || maxPrice === void 0 ? void 0 : maxPrice.value) || (priceRange === null || priceRange === void 0 ? void 0 : priceRange.max.toString()) || '∞';
...
const DefaultProductFilterSummary = ({ availableAttributes, currentFilters, priceRange, categories }) => {
...
;// ./node_modules/@evershop/evershop/dist/components/frontStore/catalog/DefaultProductFilterRender.js
...
const DefaultProductFilterRender = ({ renderProps, className = '', title = _('Filter Products'), showFilterSummary = true }) => {
    const [isMobileFilterOpen, setIsMobileFilterOpen] = (0,react.useState)(false);
    ...
    const defaultComponents = (0,react.useMemo)(() => {
        const components = [];
        if (priceRange && priceRange.min !== priceRange.max) {
            components.push({
                component: { default: DefaultPriceFilterRender },
    ...
    const contextValue = (0,react.useMemo)(() => ({ updateFilter, removeFilter, clearAllFilters }), [updateFilter, removeFilter, clearAllFilters]);
    return (react.createElement(ProductFilterDispatch.Provider, { value: contextValue },
        react.createElement("button", { onClick: () => setIsMobileFilterOpen(true), className: "md:hidden w-full flex items-center justify-center space-x-2 py-3 px-4 border border-gray-300 rounded-md bg-white hover:bg-gray-50 transition-colors" },
            react.createElement("svg", { className: "w-5 h-5", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24" },
                react.createElement("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707v4.586a1 1 0 01-.293.707l-2 2A1 1 0 0111 21v-6.586a1 1 0 00-.293-.707L4.293 7.293A1 1 0 014 6.586V4z" })),
            react.createElement("span", null, _('Filters')),
            activeFilterCount > 0 && (react.createElement("span", { className: "bg-blue-500 text-white text-xs px-2 py-1 rounded-full" }, activeFilterCount))),
        isMobileFilterOpen && (react.createElement("div", { className: "md:hidden fixed inset-0 z-50 bg-black bg-opacity-50 flex items-end" },
            react.createElement("div", { className: "w-full bg-white rounded-t-lg max-h-[90vh] flex flex-col" },
                react.createElement("div", { className: "flex items-center justify-between p-4 border-b" },
                    react.createElement("h3", { className: "font-bold text-lg" }, _('Filters')),
                    react.createElement("button", { onClick: () => setIsMobileFilterOpen(false), className: "p-2 hover:bg-gray-100 rounded-full" },
                ...
                react.createElement("div", { className: "flex-1 overflow-y-auto p-4" },
                    showFilterSummary && (react.createElement(DefaultProductFilterSummary, { availableAttributes: availableAttributes, currentFilters: currentFilters, priceRange: priceRange, categories: categories })),
                    react.createElement("div", { className: isLoading ? 'opacity-75 pointer-events-none' : '' },
                        react.createElement(common_Area, { id: "productFilter", noOuter: true, coreComponents: defaultComponents, availableAttributes: availableAttributes, priceRange: priceRange, currentFilters: currentFilters, categories: categories, setting: setting }))),
                react.createElement("div", { className: "p-4 border-t flex space-x-3" },
                    react.createElement("button", { onClick: clearAllFilters, disabled: isLoading || activeFilterCount === 0, className: "flex-1 py-2 px-4 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50 disabled:opacity-50" }, _('Clear All')),
                    react.createElement("button", { onClick: () => setIsMobileFilterOpen(false), className: "flex-1 py-2 px-4 bg-blue-600 text-white rounded-md hover:bg-blue-700" }, _('Apply Filters')))))),
        react.createElement("div", { className: `hidden md:block product-filter ${className}` },
            react.createElement("div", { className: "filter-header flex items-center justify-between mb-4" },
                title && (react.createElement("h3", { className: "font-bold text-lg flex items-center" },
                    title,
                    activeFilterCount > 0 && (react.createElement("span", { className: "ml-2 bg-blue-100 text-blue-800 text-sm px-2 py-1 rounded-full" }, activeFilterCount)),
                    isLoading && (react.createElement("div", { className: "ml-2 w-4 h-4 border-2 border-blue-600 border-t-transparent rounded-full animate-spin" })))),
                activeFilterCount > 0 && (react.createElement("button", { onClick: clearAllFilters, disabled: isLoading, className: "text-sm text-gray-600 hover:text-red-600 transition-colors disabled:opacity-50" }, _('Clear All')))),
            showFilterSummary && (react.createElement(DefaultProductFilterSummary, { availableAttributes: availableAttributes, currentFilters: currentFilters, priceRange: priceRange, categories: categories })),
            react.createElement("div", { className: isLoading ? 'opacity-75 pointer-events-none' : '' },
                react.createElement(common_Area, { id: "productFilter", noOuter: true, coreComponents: defaultComponents, availableAttributes: availableAttributes, priceRange: priceRange, currentFilters: currentFilters, categories: categories, setting: setting })))));
};
//# sourceMappingURL=DefaultProductFilterRender.js.map
;// ./node_modules/@evershop/evershop/dist/components/frontStore/catalog/CategoryProductsFilter.js
...
function CategoryProductsFilter() {
    ...
    return (react.createElement(react.Fragment, null,
        react.createElement(common_Area, { id: "beforeFilter", noOuter: true }),
        react.createElement(ProductFilter, { currentFilters: category.products.currentFilters, availableAttributes: category.availableAttributes, categories: category.children, priceRange: category.priceRange }, (renderProps) => (react.createElement(DefaultProductFilterRender, { renderProps: renderProps, title: "Product Filters", className: "my-custom-class", showFilterSummary: true }))),
        react.createElement(common_Area, { id: "afterFilter", noOuter: true })));
}
//# sourceMappingURL=CategoryProductsFilter.js.map
;// ./node_modules/@evershop/evershop/dist/components/frontStore/Pagination.js
...
const usePaginationLogic = (total, limit, initialPage, onPageChange, scrollToTop = true, scrollBehavior = 'smooth') => {
    const AppContextDispatch = useAppDispatch();
    ...
    const navigateToPage = (0,react.useCallback)(async (pageNum) => {
        if (isLoading)
            return;
        let validPage;
        if (pageNum < 1)
        ...
        try {
            ...
            onPageChange === null || onPageChange === void 0 ? void 0 : onPageChange(validPage);
        }
        catch (error) {
            setPage(page);
        }
    ...
    const goToPage = (0,react.useCallback)((pageNum) => navigateToPage(pageNum), [navigateToPage]);
    const goToNext = (0,react.useCallback)(() => navigateToPage(page + 1), [navigateToPage, page]);
    const goToPrev = (0,react.useCallback)(() => navigateToPage(page - 1), [navigateToPage, page]);
    const goToFirst = (0,react.useCallback)(() => navigateToPage(1), [navigateToPage]);
    const goToLast = (0,react.useCallback)(() => navigateToPage(totalPages), [navigateToPage, totalPages]);
    const getPageNumbers = (0,react.useCallback)((range = 5) => {
        const pages = [];
    ...
    const isCurrentPage = (0,react.useCallback)((pageNum) => pageNum === page, [page]);
    const isValidPage = (0,react.useCallback)((pageNum) => pageNum >= 1 && pageNum <= totalPages, [totalPages]);
    const getDisplayText = (0,react.useCallback)(() => {
        const start = (page - 1) * limit + 1;
        const end = Math.min(page * limit, total);
        return `Showing ${start}-${end} of ${total} results`;
    }, [page, limit, total]);
    const getPageInfo = (0,react.useCallback)(() => {
        const start = (page - 1) * limit + 1;
        const end = Math.min(page * limit, total);
        return {
            showing: `${start}-${end}`,
...
function Pagination({ total, limit, currentPage, children, onPageChange, scrollToTop = true, scrollBehavior = 'smooth' }) {
    ...
    const contextValue = react.useMemo(() => ({ goToPage: paginationLogic.goToPage }), [paginationLogic.goToPage]);
    return (react.createElement(PaginationContext.Provider, { value: contextValue }, children(renderProps)));
}
// Default pagination renderer component (maintains original design)
const DefaultPaginationRenderer = ({ renderProps, className = '', showInfo = false }) => {
    ...
    return (react.createElement("div", { className: `products-pagination ${className}` },
        showInfo && (react.createElement("div", { className: "pagination-info text-center text-gray-600 mb-4" }, getDisplayText())),
        react.createElement("ul", { className: "pagination flex justify-center space-x-3" },
            hasPrev && (react.createElement("li", { className: "page-item prev self-center" },
                react.createElement("button", { type: "button", className: "link-button page-link flex justify-center items-center", onClick: goToPrev, disabled: isLoading },
            ...
            getPageNumbers().map((p) => (react.createElement("li", { key: p, className: `page-item self-center ${isCurrentPage(p) ? 'current' : ''}` },
                react.createElement("button", { type: "button", className: "link-button page-link flex justify-center items-center", onClick: () => goToPage(p), disabled: isLoading || isCurrentPage(p) }, isCurrentPage(p) ? react.createElement("strong", null, p) : p)))),
            hasNext && (react.createElement("li", { className: "page-item next self-center" },
                react.createElement("button", { type: "button", className: "page-link link-button flex justify-center items-center", onClick: goToNext, disabled: isLoading },
                    react.createElement("svg", { xmlns: "http://www.w3.org/2000/svg", className: "h-4 w-4", viewBox: "0 0 20 20", fill: "currentColor" },
        ...
        isLoading && (react.createElement("div", { className: "pagination-loading text-center mt-2" },
            react.createElement("div", { className: "inline-block w-4 h-4 border-2 border-blue-600 border-t-transparent rounded-full animate-spin" })))));
};
// Compact pagination renderer
const CompactPaginationRenderer = ({ renderProps, className = '' }) => {
    ...
    return (React.createElement("div", { className: `compact-pagination flex items-center justify-between ${className}` },
        React.createElement("div", { className: "pagination-info text-sm text-gray-600" },
            "Showing ",
            showing,
            " of ",
            total),
        React.createElement("div", { className: "pagination-controls flex items-center space-x-2" },
            React.createElement("button", { onClick: goToPrev, disabled: !hasPrev || isLoading, className: "px-3 py-1 text-sm border border-gray-300 rounded hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed" }, "Previous"),
            React.createElement("span", { className: "text-sm text-gray-600" },
                "Page ",
                currentPage,
                " of ",
                totalPages),
            React.createElement("button", { onClick: goToNext, disabled: !hasNext || isLoading, className: "px-3 py-1 text-sm border border-gray-300 rounded hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed" }, "Next"))));
};
const InputPaginationRenderer = ({ renderProps, className = '' }) => {
    ...
    return (React.createElement("div", { className: `input-pagination ${className}` },
        React.createElement("div", { className: "text-center text-sm text-gray-600 mb-4" }, getDisplayText()),
        React.createElement("div", { className: "flex items-center justify-center space-x-4" },
            React.createElement("button", { onClick: goToFirst, disabled: !hasPrev || isLoading, className: "px-2 py-1 text-sm border border-gray-300 rounded hover:bg-gray-50 disabled:opacity-50" }, "First"),
            React.createElement("button", { onClick: goToPrev, disabled: !hasPrev || isLoading, className: "px-3 py-1 text-sm border border-gray-300 rounded hover:bg-gray-50 disabled:opacity-50" }, "\u2190"),
            React.createElement("form", { onSubmit: handleInputSubmit, className: "flex items-center space-x-2" },
                React.createElement("span", { className: "text-sm" }, "Page"),
                React.createElement("input", { type: "number", min: "1", max: totalPages, value: inputPage, onChange: (e) => setInputPage(e.target.value), className: "w-16 px-2 py-1 text-sm border border-gray-300 rounded text-center", disabled: isLoading }),
                React.createElement("span", { className: "text-sm" },
                    "of ",
                    totalPages),
                React.createElement("button", { type: "submit", className: "px-2 py-1 text-sm bg-blue-600 text-white rounded hover:bg-blue-700 disabled:opacity-50", disabled: isLoading }, "Go")),
            React.createElement("button", { onClick: goToNext, disabled: !hasNext || isLoading, className: "px-3 py-1 text-sm border border-gray-300 rounded hover:bg-gray-50 disabled:opacity-50" }, "\u2192"),
            React.createElement("button", { onClick: goToLast, disabled: !hasNext || isLoading, className: "px-2 py-1 text-sm border border-gray-300 rounded hover:bg-gray-50 disabled:opacity-50" }, "Last"))));
};
//# sourceMappingURL=Pagination.js.map
;// ./node_modules/@evershop/evershop/dist/components/frontStore/catalog/CategoryProductsPagination.js
...
function CategoryProductsPagination() {
    ...
    return (react.createElement(Pagination, { total: products.total, limit: limit ? parseInt(limit.value, 10) : 20, currentPage: parseInt((page === null || page === void 0 ? void 0 : page.value) || '1', 10) }, (paginationProps) => (react.createElement(DefaultPaginationRenderer, { renderProps: paginationProps }))));
}
//# sourceMappingURL=CategoryProductsPagination.js.map
;// ./node_modules/@evershop/evershop/dist/components/frontStore/catalog/ProductSorting.js
...
function ProductSorting({ sortOptions = defaultSortOptions, defaultSortBy = '', defaultSortOrder = 'asc', showSortDirection = true, enableUrlUpdate = true, onSortChange, renderSortSelect, renderSortDirection, className = '', disabled = false }) {
    const AppContextDispatch = useAppDispatch();
    ...
    const defaultSortChangeHandler = (0,react.useCallback)(async (newSortState) => {
        if (!enableUrlUpdate)
            return;
        const currentUrl = window.location.href;
        const url = new URL(currentUrl, window.location.origin);
    ...
    const onChangeSort = (0,react.useCallback)(async (newSortBy) => {
        if (disabled)
            return;
    ...
    const onChangeDirection = (0,react.useCallback)(async () => {
        if (disabled)
            return;
...
function CategoryView({ category }) {
    return (react.createElement(CategoryProvider, { category: category },
        react.createElement(common_Area, { id: "categoryPageTop", className: "category__page__top" }),
        react.createElement(CategoryInfo, null),
        react.createElement("div", { className: "page-width grid grid-cols-1 md:grid-cols-4 gap-5" },
            react.createElement(common_Area, { id: "categoryLeftColumn", className: "md:col-span-1", coreComponents: [
                    {
            ...
            react.createElement(common_Area, { id: "categoryRightColumn", className: "md:col-span-3", coreComponents: [
                    {
                        component: {
                            default: (react.createElement("div", { className: "flex justify-between items-center border-b border-gray-300 mb-8" },
        ...
        react.createElement(common_Area, { id: "categoryPageBottom", className: "category__page__bottom" })));
}
...
const CategoryView_fragments = (/* unused pure expression or super */ null && (`
  fragment Product on Product {
    productId
    name
    sku
...
common_Area.defaultProps.components = {
  ...
  content: {
    ...
    ef3e4b6a6293dd6e1c359f06a549dcf10: {
      id: 'ef3e4b6a6293dd6e1c359f06a549dcf10',
      sortOrder: 10,
...
/******/ 	var __webpack_exports__ = __webpack_require__.O(undefined, [4286], () => (__webpack_require__(64276)))
